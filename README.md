...
* **高度な拡張性:** レンダーグラフによるパス管理と、ECSによる柔軟なシーン管理。
* **実験場:** スペクトルレンダリング、Ray Marching、プロシージャルモデリングなどを自由に試せる基盤。
...
- must
    ...
    - [ ] Vulkan初期化 (Headless): インスタンス、物理デバイス作成
    - [ ] メモリ管理: VMAの組み込み
    - [ ] 最小描画: 画面（画像）を単色でクリアする
    - [ ] 画像出力: 描画結果をPNGファイルとして保存
